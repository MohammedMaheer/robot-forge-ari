...
oauthRouter.post('/exchange', asyncHandler(async (req: Request, res: Response) => {
  const { code } = req.body;
  ...
  // Look up user to return in response
  ...
  res.json({
    data: {
      tokens: { accessToken: entry.accessToken, expiresIn: 900 },
...
oauthRouter.get('/github/callback', passport.authenticate('github', { session: false, failureRedirect: `${FRONTEND_URL}/login?error=github_failed` }), asyncHandler(async (req: Request, res: Response) => {
...
oauthRouter.get('/google/callback', passport.authenticate('google', { session: false, failureRedirect: `${FRONTEND_URL}/login?error=google_failed` }), asyncHandler(async (req: Request, res: Response) => {
  const user = req.user as any;
  const tokens = generateTokens(user);
  await prisma.refreshToken.create({ data: { userId: user.id, token: tokens.refreshToken, expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) } });
